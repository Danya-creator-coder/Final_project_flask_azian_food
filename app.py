...
app.config['MAX_CONTENT_LENGTH'] = 5 * 1024 * 1024  # Ограничение размера файла (5MB)
...
def profile():
    ...
    # Теперь безопасно получать доступ к полям пользователя
    avatar_url = user['avatar_path'] if 'avatar_path' in user else url_for('static', filename='uploads/default.png')
    return render_template("profile.html", avatar_url=avatar_url, username=user['username'])
...
def upload_avatar():
    ...
    if not allowed_file(file.filename):
        return jsonify(success=False, message="Неверный формат файла. Разрешены только PNG, JPG, JPEG и GIF"), 400

    try:
        filename = secure_filename(f"avatar_{session['user_id']}.png")
        ...
        return jsonify(success=True, message="Аватар успешно загружен",
                       avatar_url=url_for('static', filename=f'uploads/{filename}')), 200

    except Exception as e:
        print(f"Ошибка при загрузке аватара: {str(e)}")
        return jsonify(success=False, message="Произошла ошибка при загрузке аватара"), 500
...
def api_register():
    ...
    try:
        ...
        return jsonify(success=True, message="Регистрация успешна", redirect="/profile", username=username)
    except Exception as e:
        conn.close()
...
def api_login():
    ...
    if user and check_password_hash(user['password'], password):
        ...
        return jsonify(success=True, message="Вход выполнен успешно", redirect="/profile", username=username)
    else:
